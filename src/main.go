...
func getKustomizations() ([]Kustomization, error) {
	...
	if err != nil {
		log.Errorf("❌ Erreur lors de la récupération des kustomizations: %v", err)
		return nil, err
	}
...
func getHelmReleases() ([]HelmRelease, error) {
	...
	results, err := dynamicClient.Resource(gvr).Namespace("").List(context.TODO(), metav1.ListOptions{})
	if err != nil {
		log.Errorf("❌ Erreur lors de la récupération des HelmReleases: %v", err)
		return nil, err
	}
...
func getK8sClient() (*rest.Config, error) {
	...
	if err != nil {
		...
		if err != nil {
			log.Errorf("❌ impossible de charger la configuration Kubernetes: %v", err)
...
func init() {
	// Configuration de base de logrus
...
func main() {
	// 📝 Log de démarrage
	log.Infof("🚀 Démarrage du serveur Frontend FluxCD Viewer...")
	...
	http.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		...
		_, err := getK8sClient()
		if err != nil {
			log.Errorf("❌ Health check échoué: %v", err)
	...
	// 📢 Logs plus détaillés
	log.Infof("🛣️  Routes configurées: /, /analyze, /details, /health")
	log.Infof("🌐 Serveur prêt sur http://localhost:8080")
...
func handleAnalyze(w http.ResponseWriter, r *http.Request) {
	kustomizations, err := getKustomizations()
	if err != nil {
		log.Errorf("❌ Erreur analyse: %v", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	...
	if err := json.NewEncoder(w).Encode(kustomizations); err != nil {
		log.Errorf("❌ Erreur encodage JSON: %v", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
}
...
func handleHome(w http.ResponseWriter, r *http.Request) {
	...
	funcMap := template.FuncMap{
		...
		"countHealthyHr": func(kustomizations []HelmRelease) int {
			count := 0
		...
		"countFailedHr": func(kustomizations []HelmRelease) int {
			count := 0
			for _, k := range kustomizations {
				if k.Status != "True" {
					count++
		...
		"countHealthyKs": func(kustomizations []Kustomization) int {
			count := 0
			for _, k := range kustomizations {
				if k.Status == "True" {
					count++
		...
		"countFailedKs": func(kustomizations []Kustomization) int {
			count := 0
			for _, k := range kustomizations {
				if k.Status != "True" {
					count++
	...
	if err != nil {
		log.Infof("Erreur lors de l'analyse: %v", err)
	...
	if err := tmpl.Execute(w, data); err != nil {
		log.Infof("Erreur template: %v", err)
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
}
